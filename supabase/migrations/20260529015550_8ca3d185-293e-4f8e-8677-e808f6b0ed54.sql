...
DECLARE
  ...
  v_realized_freight numeric;   -- Faturamento Realizado (Proporcional ao concluído)
  v_recent_routes json;
  v_total_distance numeric;     -- Distância Total Realizada
  v_total_tolls numeric;        -- Pedágios Totais Realizados
...
BEGIN
  ...
  SELECT 
    COALESCE(SUM(full_freight), 0),                    -- Bruto total (todos roteiros)
    COALESCE(SUM(full_freight * completion_ratio), 0),  -- Realizado (proporcional)
    COALESCE(SUM(full_km * completion_ratio), 0),       -- Distância proporcional
    COALESCE(SUM(full_tolls * completion_ratio), 0)     -- Pedágios proporcionais
  INTO v_total_freight, v_realized_freight, v_total_distance, v_total_tolls FROM per_route;

  v_result := json_build_object(
    'routes', COALESCE(v_recent_routes, '[]'::json),
    'total', v_total_deliveries,
    'byStatus', v_status_counts,
    'totalFreight', v_total_freight, -- Volta a mostrar o bruto total
    ...
    'netProfit', v_realized_freight - v_total_tolls - (CASE WHEN v_km_per_liter > 0 THEN (v_total_distance / v_km_per_liter) * v_fuel_price ELSE 0 END),
