CREATE OR REPLACE FUNCTION public.get_dashboard_stats(p_date_from date, p_date_to date, p_driver_id uuid DEFAULT NULL::uuid, p_force_self boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH eligible_routes AS (
    SELECT id, total_distance, tolls_value, freight_value FROM public.routes
  ...
  per_route AS (
    SELECT er.id,
      CASE WHEN er.total_distance IS NOT NULL AND er.total_distance > 0 THEN er.total_distance
        ELSE COALESCE(c.computed_km, 0) END AS effective_km,
      COALESCE(er.tolls_value, 0) AS tolls, COALESCE(er.freight_value, 0) AS freight
    FROM eligible_routes er LEFT JOIN computed c ON c.route_id = er.id
  )
  SELECT COALESCE(SUM(freight), 0), COALESCE(SUM(effective_km), 0), COALESCE(SUM(tolls), 0)
  ...
  v_result := json_build_object(
    ...
    'viewMode', CASE WHEN v_is_admin THEN 'admin' ELSE 'subscriber' END,
